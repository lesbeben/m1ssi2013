...
\subsection{Gestion des problèmes}
\begin{frame}{Gestion des problèmes}
\begin{block}{Problèmes rencontrés}
\begin{description}
\item[Chef de projet hospitalisé]: Réorganisation, token Java card supprimé.
\end{description}
\end{block}

\begin{block}{Problèmes évités}
\begin{description}
\item[Appréhension nouvelles technologies]: \'{E}tude des fonctionnalités utiles au projet, suivi par le responsable technique et chef de projet.
\item[Faille sécurité découverte après tests finaux]: Risque réduit par la méthode XP.
\end{description}
\end{block}
