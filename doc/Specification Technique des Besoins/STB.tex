...
%-------------------------------------------------------------------------------
\section{Objet}
Ce document définit les exigences sur l'étude et l'implémentation des systèmes d'authentification 
utilisant des mots de passe jetables ainsi que son domaine de fonctionnement.
...
{[sujet]} & {Expression du besoin client soit l'étude et implémentations des systèmes d'authentification utilisant des mots de passe jetables.}
\tabularnewline
\\
...
\begin{figure}[h]
  ...
  \href{http://tools.ietf.org/html/rfc4793}{RFC4793}, \href{http://tools.ietf.org/html/rfc6238}{RFC6238} afin d'en fournir un état de l'art le plus exhaustif possible. Une fois celui-ci terminé nous passerons \`{a} 
...
Ce protocole décrit la Génération de l'OTP par le token gràce à une demande de l'utilisateur.
\\
...
L'Authentification permet de valider la légimité d'un utilisateur; ce protocole décrit une demande d'authentification auprès du serveur pour cela l'utilisateur
...
Lors d'une demande d'authentification au serveur, soit le client est identifié, soit l'identification est un echec et dans ce cas
...
\begin{description}
 ...
 nous acceptons les versions kernel 3.x ainsi que glibc supérieur à la version 2.15.
 \item[Android] Android désigne pour notre cas toutes les versions Android 2.3.x et versions Android 4.x.
\end{description}
