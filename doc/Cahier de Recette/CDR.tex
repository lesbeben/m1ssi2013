...
\histentry{2.1}{20/02/2014}{Version post état de l'art.}
...
\begin{document}
  ...
  Afin de tester le module PAM, nous le brancherons sur plusieurs application compatibles. Nous 
utiliserons en pratique un gestionnaire de login (Slim, GDM, KDM ...) ainsi que le logiciel openSSH.
Ces applications seront installées à la fois sur des machines personnel et sur un serveur dont les 
...
des tests à une autre équipe afin que celle-ci travaille à la réparation de l'anomalie.
  ...
  Dans la pratique, la gestions des anomalies sera faite à l'aide de l'outils \emph{issue tracker} de
  l'outil d'hébergement Github. Cette outil permet la création de file de discussion associé à un bug
  de l'application. Il laisse aussi la possibilité au collaborateurs du projet de prendre en charge 
  une \emph{issue} particulière. L'ensemble des \emph{issues} ouvertes est conservé par Github ainsi
  que les fils de discussion associés permettant de parcourir l'historique des bugs du projet.
  ...
  \begin{center}
    ...
    \begin{tabular}{|c|p{5cm}|p{5cm}|p{1.5cm}|p{1.5cm}|}
      ...
      2 & Appeler le script sur un otp valide et passé l'éxecutable dans la commande time &
            Temps < 2s & EQ\_02 & OK \\
